...
export const Products = () => {
  ...
  const addProduct = () => {
    ...
    setTimeout(() => {
      setProducts([
        ...products,
        { id: Date.now(), ...newProduct, rating: 4.0 },
      ]);
      setNewProduct({
        id: 0,
        name: "",
  ...
  const updateProduct = (id: number) => {
    ...
    setTimeout(() => {
      setProducts(
        products.map((p) =>
          p.id === id ? { ...newProduct, id, rating: p.rating } : p
        )
      );
      setEditingId(null);
      setLoading(false);
  ...
  const renderStars = (rating: number) => {
    ...
    return [...Array(5)].map((_, i) => (
      <Star
        key={i}
        className={`w-4 h-4 ${
          i < fullStars ? "text-yellow-500 fill-current" : "text-gray-400"
        }`}
      />
    ));
  };
        ...
        {filteredProducts.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProducts.map((product) => {
              ...
              const isDark =
                product.color === "bg-black" || product.color === "bg-gray-700";
